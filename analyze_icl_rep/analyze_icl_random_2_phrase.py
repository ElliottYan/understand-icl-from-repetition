        ...
        for i in range(0, len(docs)):
            ...
            for kp in num_kept_phrase[:]:
                for pl in phrase_length:
                    f_path = os.path.join(output_dir, "{}_stats_{}_icl_kp_pl-{}.test.pkl".format(mp, "{}-{}".format(kp, pl), key))
                    # if os.path.exists(f_path):
                    #     print("Skip {}".format(f_path))
                    #     continue
                    stat_dict = compute_stats_dict(single_sentence_eval_2_phrase, doc, model, tokenizer, num_kept_phrase=kp, phrase_length=pl, repeat_num=args.repeat_num)
